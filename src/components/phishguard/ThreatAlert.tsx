...
export function ThreatAlert({
  ...
  const { icon: Icon, bgColor, borderColor, iconColor, titleColor } = config[level];
    ...
    <div className={`rounded-xl border-2 ${borderColor} ${bgColor} p-4 space-y-3`}>
      ...
      {indicators.length > 0 && (
        <div className="space-y-2">
          <div className="flex items-center gap-1.5">
            ...
            <h4 className="text-sm font-medium text-gray-900 dark:text-white">{t.manualAnalysis.detectedIndicators}</h4>
          </div>
      ...
      {recommendations.length > 0 && (
        <div className="space-y-2 pt-2 border-t border-gray-200 dark:border-gray-700">
          <h4 className="text-sm font-medium text-gray-900 dark:text-white">{t.manualAnalysis.recommendations}</h4>
          <ul className="space-y-1.5">
